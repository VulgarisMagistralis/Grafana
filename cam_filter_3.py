...
dbClient = InfluxDBClient('localhost' , 8086 , 'root' , 'root' , 'CAMessageLog3')
dbClient.drop_database('CAMessageLog3')
dbClient.create_database('CAMessageLog3')
...
for j in range(len(pcap_data)) :
    ...
    if has_gn_fields(str(pcap_data[j])):
        ...
        laneP= json.dumps(pcap_data[j]['_source']['layers']['etsi_its_geonetworking']['cam']['cam.CAM_element']['cam.cam_element']['cam.camParameters_element']['cam.highFrequencyContainer_tree']['cam.basicVehicleContainerHighFrequency_element']['cam.yawRate_element'] , indent = 4).strip("\"")
...
print("Total number of Messages: " + str(j))
